...
import type { CampaignLevel, GlitchKind } from '$lib/data/types';
...
export function campaign1Sketch(p: P5) {
	const combatProfile = getCampaignCombatProfile(1);
	const levels = campaign1.levels;
	...
	let bankedGold = 0;
	let waveGold = 0;
	let pixlHealth = combatProfile.pixl.health;
	let pixlFlash = 0;
	let enemyId = 0;
	...
	const startLevel = (levelIndex: number) => {
		...
		spawnAccumulator = 0;
		waveGold = 0;
		pixlHealth = combatProfile.pixl.health;
		pixlFlash = 0;
		enemyId = 0;
		enemies = [];
		projectiles = [];
	...
	const fireProjectile = (target: EnemyState) => {
		...
		projectiles.push({
			...
			damage: combatProfile.pixl.damage
		});
	};
	...
	const updateWaveFlow = (dt: number) => {
		...
		shotAccumulator += dt * combatProfile.pixl.attackSpeed;
	...
	const drawHud = () => {
		const healthRatio = pixlHealth / combatProfile.pixl.health;
		...
		p.fill(255);
		p.textSize(12);
		p.text(`CAMPAIGN 1  STAGE ${currentLevel.stage}  LEVEL ${currentLevel.stageLevel}`, 18, 16);
		p.fill(160);
		p.text(`PIXL HP ${Math.ceil(pixlHealth)} / ${combatProfile.pixl.health}`, 18, 38);
