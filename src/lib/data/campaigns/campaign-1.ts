import type { CampaignDefinition } from '$lib/data/types';

export const campaign1: CampaignDefinition = {
	campaign: 1,
	stages: 5,
	levelsPerStage: 10,
	totalLevels: 50,
	combatProfile: 'baseline-v1',
	baseline: {
		startingEnemies: 3,
		stageEnemyIncrease: 10,
		stageLevelBonusScale: 4,
		stageLevelGrowthFactor: 1.17,
		spawnRatePerSecond: 1,
		tutorialLevels: [1, 2, 3],
		compositionRules: {
			tutorial: 'biters only for levels 1-3',
			standard: {
				swarmers: 'floor(totalEnemies / 3)',
				tankers: 'floor(totalEnemies / 8)',
				biters: 'totalEnemies - swarmers - tankers'
			}
		},
		bossEnemyMultipliers: {
			stage: 1.25,
			campaign: 1.4
		},
		xpDropRules: {
			biter: 'stage + 1',
			swarmer: 'stage + 2',
			tanker: 'stage + 5'
		}
	},
	levels: [
		{
			campaign: 1,
			stage: 1,
			stageLevel: 1,
			campaignLevel: 1,
			isStageBoss: false,
			isCampaignBoss: false,
			totalEnemies: 3,
			composition: {
				biters: 3,
				swarmers: 0,
				tankers: 0
			},
			spawnRatePerSecond: 0.3,
			xpPerEnemy: {
				biter: 2,
				swarmer: 3,
				tanker: 6
			},
			totalXpReward: 6
		},
		{
			campaign: 1,
			stage: 1,
			stageLevel: 2,
			campaignLevel: 2,
			isStageBoss: false,
			isCampaignBoss: false,
			totalEnemies: 4,
			composition: {
				biters: 4,
				swarmers: 0,
				tankers: 0
			},
			spawnRatePerSecond: 0.3,
			xpPerEnemy: {
				biter: 2,
				swarmer: 3,
				tanker: 6
			},
			totalXpReward: 8
		},
		{
			campaign: 1,
			stage: 1,
			stageLevel: 3,
			campaignLevel: 3,
			isStageBoss: false,
			isCampaignBoss: false,
			totalEnemies: 4,
			composition: {
				biters: 4,
				swarmers: 0,
				tankers: 0
			},
			spawnRatePerSecond: 0.3,
			xpPerEnemy: {
				biter: 2,
				swarmer: 3,
				tanker: 6
			},
			totalXpReward: 8
		},
		{
			campaign: 1,
			stage: 1,
			stageLevel: 4,
			campaignLevel: 4,
			isStageBoss: false,
			isCampaignBoss: false,
			totalEnemies: 5,
			composition: {
				biters: 5,
				swarmers: 0,
				tankers: 0
			},
			spawnRatePerSecond: 0.3,
			xpPerEnemy: {
				biter: 2,
				swarmer: 3,
				tanker: 6
			},
			totalXpReward: 10
		},
		{
			campaign: 1,
			stage: 1,
			stageLevel: 5,
			campaignLevel: 5,
			isStageBoss: false,
			isCampaignBoss: false,
			totalEnemies: 6,
			composition: {
				biters: 4,
				swarmers: 2,
				tankers: 0
			},
			spawnRatePerSecond: 0.3,
			xpPerEnemy: {
				biter: 2,
				swarmer: 3,
				tanker: 6
			},
			totalXpReward: 14
		},
		{
			campaign: 1,
			stage: 1,
			stageLevel: 6,
			campaignLevel: 6,
			isStageBoss: false,
			isCampaignBoss: false,
			totalEnemies: 8,
			composition: {
				biters: 5,
				swarmers: 3,
				tankers: 0
			},
			spawnRatePerSecond: 1,
			xpPerEnemy: {
				biter: 2,
				swarmer: 3,
				tanker: 6
			},
			totalXpReward: 19
		},
		{
			campaign: 1,
			stage: 1,
			stageLevel: 7,
			campaignLevel: 7,
			isStageBoss: false,
			isCampaignBoss: false,
			totalEnemies: 9,
			composition: {
				biters: 6,
				swarmers: 3,
				tankers: 0
			},
			spawnRatePerSecond: 1,
			xpPerEnemy: {
				biter: 2,
				swarmer: 3,
				tanker: 6
			},
			totalXpReward: 21
		},
		{
			campaign: 1,
			stage: 1,
			stageLevel: 8,
			campaignLevel: 8,
			isStageBoss: false,
			isCampaignBoss: false,
			totalEnemies: 10,
			composition: {
				biters: 6,
				swarmers: 4,
				tankers: 0
			},
			spawnRatePerSecond: 1,
			xpPerEnemy: {
				biter: 2,
				swarmer: 3,
				tanker: 6
			},
			totalXpReward: 24
		},
		{
			campaign: 1,
			stage: 1,
			stageLevel: 9,
			campaignLevel: 9,
			isStageBoss: false,
			isCampaignBoss: false,
			totalEnemies: 12,
			composition: {
				biters: 7,
				swarmers: 4,
				tankers: 1
			},
			spawnRatePerSecond: 1,
			xpPerEnemy: {
				biter: 2,
				swarmer: 3,
				tanker: 6
			},
			totalXpReward: 26
		},
		{
			campaign: 1,
			stage: 1,
			stageLevel: 10,
			campaignLevel: 10,
			isStageBoss: true,
			isCampaignBoss: false,
			totalEnemies: 16,
			composition: {
				biters: 9,
				swarmers: 5,
				tankers: 2
			},
			spawnRatePerSecond: 1,
			xpPerEnemy: {
				biter: 2,
				swarmer: 3,
				tanker: 6
			},
			totalXpReward: 45
		},
		{
			campaign: 1,
			stage: 2,
			stageLevel: 1,
			campaignLevel: 11,
			isStageBoss: false,
			isCampaignBoss: false,
			totalEnemies: 15,
			composition: {
				biters: 9,
				swarmers: 5,
				tankers: 1
			},
			spawnRatePerSecond: 1,
			xpPerEnemy: {
				biter: 3,
				swarmer: 4,
				tanker: 7
			},
			totalXpReward: 54
		},
		{
			campaign: 1,
			stage: 2,
			stageLevel: 2,
			campaignLevel: 12,
			isStageBoss: false,
			isCampaignBoss: false,
			totalEnemies: 16,
			composition: {
				biters: 9,
				swarmers: 5,
				tankers: 2
			},
			spawnRatePerSecond: 1,
			xpPerEnemy: {
				biter: 3,
				swarmer: 4,
				tanker: 7
			},
			totalXpReward: 61
		},
		{
			campaign: 1,
			stage: 2,
			stageLevel: 3,
			campaignLevel: 13,
			isStageBoss: false,
			isCampaignBoss: false,
			totalEnemies: 16,
			composition: {
				biters: 9,
				swarmers: 5,
				tankers: 2
			},
			spawnRatePerSecond: 1,
			xpPerEnemy: {
				biter: 3,
				swarmer: 4,
				tanker: 7
			},
			totalXpReward: 61
		},
		{
			campaign: 1,
			stage: 2,
			stageLevel: 4,
			campaignLevel: 14,
			isStageBoss: false,
			isCampaignBoss: false,
			totalEnemies: 17,
			composition: {
				biters: 10,
				swarmers: 5,
				tankers: 2
			},
			spawnRatePerSecond: 1,
			xpPerEnemy: {
				biter: 3,
				swarmer: 4,
				tanker: 7
			},
			totalXpReward: 64
		},
		{
			campaign: 1,
			stage: 2,
			stageLevel: 5,
			campaignLevel: 15,
			isStageBoss: false,
			isCampaignBoss: false,
			totalEnemies: 18,
			composition: {
				biters: 10,
				swarmers: 6,
				tankers: 2
			},
			spawnRatePerSecond: 1,
			xpPerEnemy: {
				biter: 3,
				swarmer: 4,
				tanker: 7
			},
			totalXpReward: 68
		},
		{
			campaign: 1,
			stage: 2,
			stageLevel: 6,
			campaignLevel: 16,
			isStageBoss: false,
			isCampaignBoss: false,
			totalEnemies: 20,
			composition: {
				biters: 12,
				swarmers: 6,
				tankers: 2
			},
			spawnRatePerSecond: 1,
			xpPerEnemy: {
				biter: 3,
				swarmer: 4,
				tanker: 7
			},
			totalXpReward: 74
		},
		{
			campaign: 1,
			stage: 2,
			stageLevel: 7,
			campaignLevel: 17,
			isStageBoss: false,
			isCampaignBoss: false,
			totalEnemies: 21,
			composition: {
				biters: 12,
				swarmers: 7,
				tankers: 2
			},
			spawnRatePerSecond: 1,
			xpPerEnemy: {
				biter: 3,
				swarmer: 4,
				tanker: 7
			},
			totalXpReward: 78
		},
		{
			campaign: 1,
			stage: 2,
			stageLevel: 8,
			campaignLevel: 18,
			isStageBoss: false,
			isCampaignBoss: false,
			totalEnemies: 23,
			composition: {
				biters: 14,
				swarmers: 7,
				tankers: 2
			},
			spawnRatePerSecond: 1,
			xpPerEnemy: {
				biter: 3,
				swarmer: 4,
				tanker: 7
			},
			totalXpReward: 84
		},
		{
			campaign: 1,
			stage: 2,
			stageLevel: 9,
			campaignLevel: 19,
			isStageBoss: false,
			isCampaignBoss: false,
			totalEnemies: 25,
			composition: {
				biters: 14,
				swarmers: 8,
				tankers: 3
			},
			spawnRatePerSecond: 1,
			xpPerEnemy: {
				biter: 3,
				swarmer: 4,
				tanker: 7
			},
			totalXpReward: 95
		},
		{
			campaign: 1,
			stage: 2,
			stageLevel: 10,
			campaignLevel: 20,
			isStageBoss: true,
			isCampaignBoss: false,
			totalEnemies: 34,
			composition: {
				biters: 19,
				swarmers: 11,
				tankers: 4
			},
			spawnRatePerSecond: 1,
			xpPerEnemy: {
				biter: 3,
				swarmer: 4,
				tanker: 7
			},
			totalXpReward: 129
		},
		{
			campaign: 1,
			stage: 3,
			stageLevel: 1,
			campaignLevel: 21,
			isStageBoss: false,
			isCampaignBoss: false,
			totalEnemies: 25,
			composition: {
				biters: 14,
				swarmers: 8,
				tankers: 3
			},
			spawnRatePerSecond: 1,
			xpPerEnemy: {
				biter: 4,
				swarmer: 5,
				tanker: 8
			},
			totalXpReward: 120
		},
		{
			campaign: 1,
			stage: 3,
			stageLevel: 2,
			campaignLevel: 22,
			isStageBoss: false,
			isCampaignBoss: false,
			totalEnemies: 26,
			composition: {
				biters: 15,
				swarmers: 8,
				tankers: 3
			},
			spawnRatePerSecond: 1,
			xpPerEnemy: {
				biter: 4,
				swarmer: 5,
				tanker: 8
			},
			totalXpReward: 124
		},
		{
			campaign: 1,
			stage: 3,
			stageLevel: 3,
			campaignLevel: 23,
			isStageBoss: false,
			isCampaignBoss: false,
			totalEnemies: 26,
			composition: {
				biters: 15,
				swarmers: 8,
				tankers: 3
			},
			spawnRatePerSecond: 1,
			xpPerEnemy: {
				biter: 4,
				swarmer: 5,
				tanker: 8
			},
			totalXpReward: 124
		},
		{
			campaign: 1,
			stage: 3,
			stageLevel: 4,
			campaignLevel: 24,
			isStageBoss: false,
			isCampaignBoss: false,
			totalEnemies: 27,
			composition: {
				biters: 15,
				swarmers: 9,
				tankers: 3
			},
			spawnRatePerSecond: 1,
			xpPerEnemy: {
				biter: 4,
				swarmer: 5,
				tanker: 8
			},
			totalXpReward: 129
		},
		{
			campaign: 1,
			stage: 3,
			stageLevel: 5,
			campaignLevel: 25,
			isStageBoss: false,
			isCampaignBoss: false,
			totalEnemies: 28,
			composition: {
				biters: 16,
				swarmers: 9,
				tankers: 3
			},
			spawnRatePerSecond: 1,
			xpPerEnemy: {
				biter: 4,
				swarmer: 5,
				tanker: 8
			},
			totalXpReward: 133
		},
		{
			campaign: 1,
			stage: 3,
			stageLevel: 6,
			campaignLevel: 26,
			isStageBoss: false,
			isCampaignBoss: false,
			totalEnemies: 30,
			composition: {
				biters: 17,
				swarmers: 10,
				tankers: 3
			},
			spawnRatePerSecond: 1,
			xpPerEnemy: {
				biter: 4,
				swarmer: 5,
				tanker: 8
			},
			totalXpReward: 142
		},
		{
			campaign: 1,
			stage: 3,
			stageLevel: 7,
			campaignLevel: 27,
			isStageBoss: false,
			isCampaignBoss: false,
			totalEnemies: 31,
			composition: {
				biters: 18,
				swarmers: 10,
				tankers: 3
			},
			spawnRatePerSecond: 1,
			xpPerEnemy: {
				biter: 4,
				swarmer: 5,
				tanker: 8
			},
			totalXpReward: 146
		},
		{
			campaign: 1,
			stage: 3,
			stageLevel: 8,
			campaignLevel: 28,
			isStageBoss: false,
			isCampaignBoss: false,
			totalEnemies: 33,
			composition: {
				biters: 18,
				swarmers: 11,
				tankers: 4
			},
			spawnRatePerSecond: 1,
			xpPerEnemy: {
				biter: 4,
				swarmer: 5,
				tanker: 8
			},
			totalXpReward: 159
		},
		{
			campaign: 1,
			stage: 3,
			stageLevel: 9,
			campaignLevel: 29,
			isStageBoss: false,
			isCampaignBoss: false,
			totalEnemies: 35,
			composition: {
				biters: 20,
				swarmers: 11,
				tankers: 4
			},
			spawnRatePerSecond: 1,
			xpPerEnemy: {
				biter: 4,
				swarmer: 5,
				tanker: 8
			},
			totalXpReward: 167
		},
		{
			campaign: 1,
			stage: 3,
			stageLevel: 10,
			campaignLevel: 30,
			isStageBoss: true,
			isCampaignBoss: false,
			totalEnemies: 46,
			composition: {
				biters: 26,
				swarmers: 15,
				tankers: 5
			},
			spawnRatePerSecond: 1,
			xpPerEnemy: {
				biter: 4,
				swarmer: 5,
				tanker: 8
			},
			totalXpReward: 219
		},
		{
			campaign: 1,
			stage: 4,
			stageLevel: 1,
			campaignLevel: 31,
			isStageBoss: false,
			isCampaignBoss: false,
			totalEnemies: 35,
			composition: {
				biters: 20,
				swarmers: 11,
				tankers: 4
			},
			spawnRatePerSecond: 1.5,
			xpPerEnemy: {
				biter: 5,
				swarmer: 6,
				tanker: 9
			},
			totalXpReward: 202
		},
		{
			campaign: 1,
			stage: 4,
			stageLevel: 2,
			campaignLevel: 32,
			isStageBoss: false,
			isCampaignBoss: false,
			totalEnemies: 36,
			composition: {
				biters: 20,
				swarmers: 12,
				tankers: 4
			},
			spawnRatePerSecond: 1.5,
			xpPerEnemy: {
				biter: 5,
				swarmer: 6,
				tanker: 9
			},
			totalXpReward: 208
		},
		{
			campaign: 1,
			stage: 4,
			stageLevel: 3,
			campaignLevel: 33,
			isStageBoss: false,
			isCampaignBoss: false,
			totalEnemies: 36,
			composition: {
				biters: 20,
				swarmers: 12,
				tankers: 4
			},
			spawnRatePerSecond: 1.5,
			xpPerEnemy: {
				biter: 5,
				swarmer: 6,
				tanker: 9
			},
			totalXpReward: 208
		},
		{
			campaign: 1,
			stage: 4,
			stageLevel: 4,
			campaignLevel: 34,
			isStageBoss: false,
			isCampaignBoss: false,
			totalEnemies: 37,
			composition: {
				biters: 21,
				swarmers: 12,
				tankers: 4
			},
			spawnRatePerSecond: 1.5,
			xpPerEnemy: {
				biter: 5,
				swarmer: 6,
				tanker: 9
			},
			totalXpReward: 213
		},
		{
			campaign: 1,
			stage: 4,
			stageLevel: 5,
			campaignLevel: 35,
			isStageBoss: false,
			isCampaignBoss: false,
			totalEnemies: 38,
			composition: {
				biters: 22,
				swarmers: 12,
				tankers: 4
			},
			spawnRatePerSecond: 1.5,
			xpPerEnemy: {
				biter: 5,
				swarmer: 6,
				tanker: 9
			},
			totalXpReward: 218
		},
		{
			campaign: 1,
			stage: 4,
			stageLevel: 6,
			campaignLevel: 36,
			isStageBoss: false,
			isCampaignBoss: false,
			totalEnemies: 40,
			composition: {
				biters: 22,
				swarmers: 13,
				tankers: 5
			},
			spawnRatePerSecond: 1.5,
			xpPerEnemy: {
				biter: 5,
				swarmer: 6,
				tanker: 9
			},
			totalXpReward: 233
		},
		{
			campaign: 1,
			stage: 4,
			stageLevel: 7,
			campaignLevel: 37,
			isStageBoss: false,
			isCampaignBoss: false,
			totalEnemies: 41,
			composition: {
				biters: 23,
				swarmers: 13,
				tankers: 5
			},
			spawnRatePerSecond: 1.5,
			xpPerEnemy: {
				biter: 5,
				swarmer: 6,
				tanker: 9
			},
			totalXpReward: 238
		},
		{
			campaign: 1,
			stage: 4,
			stageLevel: 8,
			campaignLevel: 38,
			isStageBoss: false,
			isCampaignBoss: false,
			totalEnemies: 43,
			composition: {
				biters: 24,
				swarmers: 14,
				tankers: 5
			},
			spawnRatePerSecond: 1.5,
			xpPerEnemy: {
				biter: 5,
				swarmer: 6,
				tanker: 9
			},
			totalXpReward: 249
		},
		{
			campaign: 1,
			stage: 4,
			stageLevel: 9,
			campaignLevel: 39,
			isStageBoss: false,
			isCampaignBoss: false,
			totalEnemies: 45,
			composition: {
				biters: 25,
				swarmers: 15,
				tankers: 5
			},
			spawnRatePerSecond: 1.5,
			xpPerEnemy: {
				biter: 5,
				swarmer: 6,
				tanker: 9
			},
			totalXpReward: 260
		},
		{
			campaign: 1,
			stage: 4,
			stageLevel: 10,
			campaignLevel: 40,
			isStageBoss: true,
			isCampaignBoss: false,
			totalEnemies: 59,
			composition: {
				biters: 33,
				swarmers: 19,
				tankers: 7
			},
			spawnRatePerSecond: 1.5,
			xpPerEnemy: {
				biter: 5,
				swarmer: 6,
				tanker: 9
			},
			totalXpReward: 342
		},
		{
			campaign: 1,
			stage: 5,
			stageLevel: 1,
			campaignLevel: 41,
			isStageBoss: false,
			isCampaignBoss: false,
			totalEnemies: 45,
			composition: {
				biters: 25,
				swarmers: 15,
				tankers: 5
			},
			spawnRatePerSecond: 1.5,
			xpPerEnemy: {
				biter: 6,
				swarmer: 7,
				tanker: 10
			},
			totalXpReward: 305
		},
		{
			campaign: 1,
			stage: 5,
			stageLevel: 2,
			campaignLevel: 42,
			isStageBoss: false,
			isCampaignBoss: false,
			totalEnemies: 46,
			composition: {
				biters: 26,
				swarmers: 15,
				tankers: 5
			},
			spawnRatePerSecond: 1.5,
			xpPerEnemy: {
				biter: 6,
				swarmer: 7,
				tanker: 10
			},
			totalXpReward: 311
		},
		{
			campaign: 1,
			stage: 5,
			stageLevel: 3,
			campaignLevel: 43,
			isStageBoss: false,
			isCampaignBoss: false,
			totalEnemies: 46,
			composition: {
				biters: 26,
				swarmers: 15,
				tankers: 5
			},
			spawnRatePerSecond: 1.5,
			xpPerEnemy: {
				biter: 6,
				swarmer: 7,
				tanker: 10
			},
			totalXpReward: 311
		},
		{
			campaign: 1,
			stage: 5,
			stageLevel: 4,
			campaignLevel: 44,
			isStageBoss: false,
			isCampaignBoss: false,
			totalEnemies: 47,
			composition: {
				biters: 27,
				swarmers: 15,
				tankers: 5
			},
			spawnRatePerSecond: 1.5,
			xpPerEnemy: {
				biter: 6,
				swarmer: 7,
				tanker: 10
			},
			totalXpReward: 317
		},
		{
			campaign: 1,
			stage: 5,
			stageLevel: 5,
			campaignLevel: 45,
			isStageBoss: false,
			isCampaignBoss: false,
			totalEnemies: 48,
			composition: {
				biters: 26,
				swarmers: 16,
				tankers: 6
			},
			spawnRatePerSecond: 1.5,
			xpPerEnemy: {
				biter: 6,
				swarmer: 7,
				tanker: 10
			},
			totalXpReward: 328
		},
		{
			campaign: 1,
			stage: 5,
			stageLevel: 6,
			campaignLevel: 46,
			isStageBoss: false,
			isCampaignBoss: false,
			totalEnemies: 50,
			composition: {
				biters: 28,
				swarmers: 16,
				tankers: 6
			},
			spawnRatePerSecond: 1.5,
			xpPerEnemy: {
				biter: 6,
				swarmer: 7,
				tanker: 10
			},
			totalXpReward: 340
		},
		{
			campaign: 1,
			stage: 5,
			stageLevel: 7,
			campaignLevel: 47,
			isStageBoss: false,
			isCampaignBoss: false,
			totalEnemies: 51,
			composition: {
				biters: 28,
				swarmers: 17,
				tankers: 6
			},
			spawnRatePerSecond: 1.5,
			xpPerEnemy: {
				biter: 6,
				swarmer: 7,
				tanker: 10
			},
			totalXpReward: 347
		},
		{
			campaign: 1,
			stage: 5,
			stageLevel: 8,
			campaignLevel: 48,
			isStageBoss: false,
			isCampaignBoss: false,
			totalEnemies: 53,
			composition: {
				biters: 30,
				swarmers: 17,
				tankers: 6
			},
			spawnRatePerSecond: 1.5,
			xpPerEnemy: {
				biter: 6,
				swarmer: 7,
				tanker: 10
			},
			totalXpReward: 359
		},
		{
			campaign: 1,
			stage: 5,
			stageLevel: 9,
			campaignLevel: 49,
			isStageBoss: false,
			isCampaignBoss: false,
			totalEnemies: 55,
			composition: {
				biters: 31,
				swarmers: 18,
				tankers: 6
			},
			spawnRatePerSecond: 1.5,
			xpPerEnemy: {
				biter: 6,
				swarmer: 7,
				tanker: 10
			},
			totalXpReward: 372
		},
		{
			campaign: 1,
			stage: 5,
			stageLevel: 10,
			campaignLevel: 50,
			isStageBoss: true,
			isCampaignBoss: true,
			totalEnemies: 80,
			composition: {
				biters: 44,
				swarmers: 26,
				tankers: 10
			},
			spawnRatePerSecond: 1.5,
			xpPerEnemy: {
				biter: 6,
				swarmer: 7,
				tanker: 10
			},
			totalXpReward: 546
		}
	]
};
